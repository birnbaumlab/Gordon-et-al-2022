...
dataset_filename = '{}dataset_processed_0001_CPcounts.xlsx'.format(base)
selections = ['Unselected','EGFP+ sorted','CD69+ R1','CD69+ R2','CD69+ R3','CD69+PD-1- R1','CD69+PD-1- R2']

# sequences to look for
seqs = {'Consensus Domains':['4-1BB,CD3z','CD28,CD3z','CD28,4-1BB,CD3z'],
        'Domains':['4-1BB','CD28','CD3z'],
        'Barcodes':['TAGGTTTAACATGTGTAG',
                    'GACGGTCGCACACGATTT',
                    'CAACAAAGAGATGTTATC',
                    'CCCGTTGAGGTGATGCAC',
                    'TAGGTTTAACATGTGTAG',
...
'''
         'CD69+ barcodes': ['TAGGTTTAACATGTGTAG',
                            'GACGGTCGCACACGATTT',
                            'CAACAAAGAGATGTTATC',
                            'CCCGTTGAGGTGATGCAC',
                            'TAGGTTTAACATGTGTAG',
                            'ATGCACACCGCTTACCGT']}
        'CD69+PD-1- barcodes':['GGTTAGTTGGAGGAGATC',
                               'TAGGTTTAACATGTGTAG',
                               'GACGGTCGCACACGATTT',
                               'TTGGGTAGGGTAGCCTAG',
                               'GGTGATAGCATTCGGCAC']}
...
def findseq(df,seq,col):
    ...
    if col == 'ICDs':
        found = df.loc[(df['ICD 1'] == seq) | (df['ICD 2'] == seq) | (df['ICD 3'] == seq)]
    else:
...
tracking = {key:[0]*len(selections) for key in seqs['Consensus Domains'] + seqs['Domains'] + seqs['Barcodes']}
...
for i,sel in enumerate(selections):
    ...
    if 'Domains' in seqs:
        for v in seqs['Domains']:
            
            found = findseq(df,v,'ICDs')
            tracking[v][i] = sum(found['Frequency'])
            #print('{} frequencies: {}'.format(v,tracking[v]))
    
    if 'Barcodes' in seqs:
        for v in seqs['Barcodes']:
